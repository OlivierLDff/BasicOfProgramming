...
ListEl * InsertAtTheEnd(ListEl * this, const int data)
{
    ListEl * it = this;
    while(it && it->nxt)
    {
    ...
    if(!u)
    {
        printf("fail to allocate aelement\n");
        return this; //be carful memory leak may occur
    }
    if(this)
        it->nxt = u;
    else
...
ListEl * Search(ListEl * this, const int data)
{
    ListEl * it = this;
    while(it)
    {
