...
BTree * findValue(BTree * this, const int val)
{
	BTree * res = NULL;
	if(this)
	{
		if (val == this->value) return this;
		if ((res = findValue(this->left, val))) return res;
		if ((res = findValue(this->right, val))) return res;
	}
	return res;
}
...
int main(void) {
	...
	printf("find value 15 is %p\n", findValue(root, 15));
	printf("find value 0 is %p\n", findValue(root, 0));
	destroyTree(root);
