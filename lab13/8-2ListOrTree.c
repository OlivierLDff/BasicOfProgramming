...
{
	const int res = this ? getSumOfValue(this->left) + getSumOfValue(this->right) + this->value : 0;
...
int getIsList(BTree* this)
{
	return this ? (this->right && !this->left && getIsList(this->right) ? 1 : 0) || (!this->right && this->left && getIsList(this->left) ? 1 : 0) || !this->right && !this->left : 1;
}
...
int main(void) {
	...
	root->right = malloc(sizeof(BTree));
	root->right->left = 0;
	root->right->right = 0;

	root->right->left = malloc(sizeof(BTree));
